...
class ocean_remap(object):
    ...
    def remap_var(self, src_var, fill_value=1.e20):
        """
...
def copy_gAttr(fptr_in, fptr_out):
    """
    copy all global attributes from one netCDF file to another
    """
    for att_name in fptr_in.ncattrs():
        if att_name == 'grid_label':
            setattr(fptr_out, att_name, 'gr')
        elif att_name == 'grid':
            setattr(fptr_out, att_name, 'ocean data regridded from native gx1v7 displaced pole grid (384x320 latxlon) to 180x360 latxlon using conservative regridding')
        elif att_name == 'nominal_resolution':
            setattr(fptr_out, att_name, '1x1 degree')
        elif att_name == 'tracking_id':
            setattr(fptr_out, att_name, "hdl:21.14100/"+str(uuid.uuid4())) 
        else:
            setattr(fptr_out, att_name, getattr(fptr_in, att_name))
...
#    for att_name in ('bounds', 'calendar', 'long_name', 'units'):
    ...
    varid_in = fptr_in.variables['time_bnds']
    varid_out = fptr_out.createVariable('time_bnds', 'f8', ('time', 'd2'))
